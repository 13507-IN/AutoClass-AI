...
export function generateSubmission(assignment) {
  const title = assignment?.title?.toLowerCase() || "";
  const description = assignment?.description?.toLowerCase() || "";

  if (title.includes("data structures") || description.includes("linked list")) {
    return linkedListText;
  }

  if (title.includes("dbms") || description.includes("normalization")) {
    return normalizationText;
  }

  return `This submission provides a structured explanation based on the assignment prompt: ${assignment?.title || "General topic"}. It includes clear definitions, key concepts, examples, and practical applications. The response is organized into sections that highlight the core idea, the important steps or components, and the real-world significance. By following a logical flow and using consistent terminology, the write-up aims to be both easy to read and academically complete. If a specific example is required, it can be extended with domain details from the classroom requirements.`;
}
